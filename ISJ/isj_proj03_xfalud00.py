...
def match_permutations(string, words):
    ...
    matching_perms = sorted([''.join(x) for x in perms & words]) # vase reseni
...
def plur2sing(singular, plural):
    ...
    # inversion using zip
    pl2sg = dict((v, k) for k, v in sg2pl.items()) # vase reseni
    return pl2sg

def vect2word(word2vect):
    v2w = {(v[0], v[1]): k for k, v in word2vect.items()} # vase reseni
    return v2w
